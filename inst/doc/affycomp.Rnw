...
assay each targeted transcript.  This is  
a bonanza for the statistical geneticist, offering a great opportunity
to create probeset summaries with
...
The benchmark data used is freely
available to the public.  The assessment data sets are the following:
...
line (CNS), were hybridized  
to human arrays (HG-U95Av2) in a range of dilutions and proportions
\cite{RMA}. 
...
concentrations. The cRNAs were  
spiked-in at a different concentration on each array (apart from
replicates) arranged in a cyclic Latin square design with each
concentration appearing once in each row and column. All arrays had a
common background cRNA. The data can be obtained form
...
R> s <- read.dilution("sifilename.csv")
...
\verb+exprSet+s created using the RMA and MAS 5.0 methods. 
...
@
\end{center}
\caption{Figure 1: The MA plot shows log fold change as a function of
...
@
\end{center}
  \caption{Figure 2:   For each gene,
...
@
\end{center}
\caption{Figure 3: This plot, using the GeneLogic dilution data,
...
@
\end{center}
\caption{Figure 4: a) Average observed $log_2$ intensity plotted
...
@
\end{center}
  \caption{Figure 5: A typical identification rule for differential
      expression filters
...
@
\end{center}
  \caption{Figure 6: a) Observed log fold changes plotted against
...
estimates. For this to work the \verb+exprSet+ object used for the
...
@
\end{center}
\caption{Figure 7:  Using the replicates from the dilution data, we calculate the mean
  ...
  = \sum_r (y_{tdrg} - y_{td\cdot g})^2/4$ are calculated as
  well. These boxplots are of the log ratios of the predicted and  
  observed variance.
}
\end{figure}
